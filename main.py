...
origins = [
    ...
    "http://localhost", "http://localhost:8080", "http://localhost:8081","https://businessworkfront.herokuapp.com/"
]
app.add_middleware(
...
@app.post("/users/{user_user}/documents/", response_model=schemas.Documents)
async def create_document_for_user(
 user_user: str, document: schemas.DocumentsCreate, db: Session = Depends(get_db)
):
    return crud.create_user_document(db=db, document=document, user_user=user_user)
